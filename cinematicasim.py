...
def v_funcao_vert():
    ...
    simular = input("[1] para iniciar simulação\n[Enter] para voltar\n--->")
    if simular == '1':
        simulacao_v_funcao_vert(vo, inter_temp)
    else:
        return
    voltar()
...
def s_funcao_tempo_vert():
    ...
    if s <= 0:
        s = 0
        print(f'Altura = {s} (chão)')
    else:
        print(f'Altura = {s}')
    simular = input("[1] para iniciar simulação\n[Enter] para voltar\n--->")
    if simular == '1':
        simulacao_sfuncaotempo_vert(so, vo, inter_temp)
    else:
        return
    voltar()
...
#tempo função da posição vertical:
...
def vert():
    ...
    print("\n[2] Lançamento para cima (Função horária da posição em função do tempo no movimento vertical)")
...
def simulacao_torricellia(vo,v,dist_perc):
    ...
    sleep(0.5)
    print(f'v = {obj.velocidade.x} ')
#....................................simulações movimento vertical:.................................
def simulacao_v_funcao_vert(vo,inter_temp):
    a = 9.8
    #cena:
    scene.background = color.hsv_to_rgb(vector(0.1,0.1,0.2))
    scene.autoscale = False
    scene.width = 1080
    scene.height = 600
    scene.append_to_title("<b>Função horária da velocidade no movimento vertical</b>")
    scene.append_to_caption(f'\tInformações:\n\tVelocidade inicial: {vo}\n\tAceleração: {a}m/s\n\tTempo: {inter_temp}')
    scene.align = 'left'
    
    obj = sphere(pos=vector(0,0,0),velocidade=vector(0,vo,0),aceleracao=vector(0,a,0),color=color.red,make_trail=True,trail_color=color.white)
    #chao = box(pos=vector(0,0,0),color=color.green,size=vector(50,0.5,1))
    ...
    textoaceleracao = label(pos=flechaaceleracao.pos,text=f'a = {a}²',color=color.white,opacity=0,box=False)
    textovelocidade = label(text=f'V ={obj.velocidade.y}',color=color.white,opacity=0,box=False)
    dt=0.01
    t=0
    ...
    while t < inter_temp:
        ...
        textoaceleracao.pos.x = flechaaceleracao.pos.x + 5
        textoaceleracao.pos.y = flechaaceleracao.pos.y
        textoaceleracao.text = f'a = {obj.aceleracao.y}²'

        textovelocidade.pos = textoaceleracao.pos
        textovelocidade.pos.y = textoaceleracao.pos.y - 5
        textovelocidade.pos.x = textoaceleracao.pos.x + 2
        textovelocidade.text = f'V = {-obj.velocidade.y}' 
        #flecha da velocidade:
        flecha.pos=obj.pos
        flecha.pos.x = obj.pos.x - 0.5
        flecha.pos.y=obj.pos.y-0.7
        flecha.axis=obj.velocidade*0.5 #Para onde a flecha aponta
        #flecha da aceleração:
        flechaaceleracao.pos=obj.pos
        flechaaceleracao.pos.x = obj.pos.x + 0.5
        flechaaceleracao.pos.y=obj.pos.y - 0.7
        flechaaceleracao.axis=-obj.aceleracao*0.5
        scene.camera.follow(obj) #cenario
        t=t+dt
...
def simulacao_sfuncaotempo_vert(so,vo,inter_temp):
    a = 9.8
    #cena:
    scene.background = color.hsv_to_rgb(vector(0.1,0.1,0.2))
    scene.autoscale = False
    scene.width = 1080
    scene.height = 600
    scene.append_to_title("<b>Função horária da posição em função do tempo no movimento vertical</b>")
    scene.append_to_caption(f'\tInformações:\n\tPosição inicial: {so}\n\tVelocidade inicial: {vo}\n\tAceleração: {a}m/s\n\tTempo: {inter_temp}')
    scene.align = 'left'
    
    obj = sphere(pos=vector(0,so,0),velocidade=vector(0,vo,0),aceleracao=vector(0,a,0),color=color.red,make_trail=True,trail_color=color.white)
    chao = box(pos=vector(0,0,0),color=color.green,size=vector(50,0.5,1))
    
    
    flecha = arrow(pos=obj.pos,axis=obj.velocidade,color=color.blue,round=True,shaftwidth=0.2)
    flechaaceleracao = arrow(pos=obj.pos,axis=-obj.aceleracao,color=color.yellow,round=True,shaftwidth=0.2)
    texto = label(pos=vector(obj.pos.x,obj.pos.y + 3,0),text=f'S = {obj.pos.y}',color=color.white,opacity=0,box=False)
    textoaceleracao = label(pos=flechaaceleracao.pos,text=f'a = {a}²',color=color.white,opacity=0,box=False)
    textovelocidade = label(text=f'V ={obj.velocidade.y}',color=color.white,opacity=0,box=False)
    dt=0.01
    t=0
    ...
    while t < inter_temp:
        ...
        while obj.pos.y <= chao.pos.y:
            obj.velocidade = 0
            obj.pos.y = 0

        #labels:
        texto.pos.y = obj.pos.y+3 # posição da label informando
        texto.text = f'S = {obj.pos.y}' #texto da label
